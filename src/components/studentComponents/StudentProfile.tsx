...
const StudentProfile = () => {
  ...
  const upcomingOnly = allClasses
    .filter((c: UpcomingClass) => c.epoch && c.epoch >= todayTimestamp)
  ...
  const goPrevUpcoming = () => setUpcomingPage((p: number) => Math.max(1, p - 1));
  const goNextUpcoming = () => setUpcomingPage((p: number) => Math.min(upcomingTotalPages, p + 1));
  const goPrevSchedule = () => setSchedulePage((p: number) => Math.max(1, p - 1));
  const goNextSchedule = () => setSchedulePage((p: number) => Math.min(scheduleTotalPages, p + 1));
    ...
    const fetchStudentAttendance = async () => {
      ...
      try {
        ...
        if (attendanceResponse && attendanceResponse.success && attendanceResponse.data) {
          ...
          setStudentData((prev: typeof studentData) => ({
            ...prev,
