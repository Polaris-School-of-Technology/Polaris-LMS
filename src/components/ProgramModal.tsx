...
const ProgramModal: React.FC<ProgramModalProps> = ({
    ...
    const fetchReschedules = async () => {
      try {
        ...
        if (Number.isNaN(courseId)) {
          console.warn(
            "Program id/course_id is not numeric; skipping reschedule fetch:",
            candidate
          );
          setReschedules([]);
          return;
        ...
        if (
          ...
          alert(
            "Failed to create program: " +
              (result?.error || JSON.stringify(result) || "Unknown error")
          );
        }
      } catch (error: any) {
  ...
  console.log("reschedules: ", reschedules);
            ...
            >
                ...
                <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
                  ...
                  {mode === "view" ? (
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditData({ ...editData, cohort: e.target.value })
                      }
                      className="w-full h-12 px-4 bg-gray-800 border border-gray-700 text-white rounded-lg focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 focus:outline-none transition-all duration-200"
                    ...
                    >
                      ...
                      {batches.map((batch) => (
                        <option key={batch.id} value={batch.id}>
                          {batch.batch_name} ({batch.academic_year} - Semester{" "}
                          {batch.semester})
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
                    ...
                    >
                      <option value="">
                        {loading
                          ? "Loading faculties..."
                          : `Select Faculty (${faculties.length} available)`}
                      </option>
                ...
                <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50 relative">
                  ...
                  {mode === "view" ? (
                    ...
                    >
                      <span
                        className={
                          editData.assignedMentor
                            ? "text-white"
                            : "text-gray-400"
                        }
                      >
                        {editData.assignedMentor
                          ? (editData.assignedMentor as any).name
                          : "Select Mentor"}
                      </span>
                      <svg
                        ...
                        {filteredMentors.map((mentor) => (
                          ...
                          >
                            <div className="flex items-center">
                              <div
                                className={`w-2.5 h-2.5 rounded-full mr-3 ${getStatusColor(
                            ...
                            {(editData.assignedMentor as any)?.id ===
                              mentor.user_id && (
                              <svg
                    ...
                    //   id="assignedMentors"
                    //   className="h-12 flex items-center px-4 bg-gray-800 rounded-lg text-white w-full"
                    // >
                    //   {reschedules.map((reschedule) => (
                    //     <option key={reschedule.id} value={reschedule.id}>
                  ...
                  {mode === "view" ? (
                    <div className="h-12 flex items-center px-4 bg-gray-800 rounded-lg">
                      <span className="text-2xl font-bold text-white">
                        {program?.sessions ?? (program as any)?.sessions ?? 0}
                      </span>
                    </div>
                  ) : (
                    <input
                      type="number"
                      value={
                        typeof editData.sessions === "number"
                          ? editData.sessions
                          : 0
                      }
                      onChange={(e) =>
                        setEditData({
                          ...editData,
                          sessions: parseInt(e.target.value, 10) || 0,
                        })
                      }
                      className="w-full h-12 px-4 bg-gray-800 border border-gray-700 text-white rounded-lg focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 focus:outline-none transition-all duration-200"
                      placeholder="0"
                      ...
                      <span
                        className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${
                          ((program as any)?.status ??
                            (program as any)?.active) === "active"
                            ? "text-green-400 bg-green-400/10"
                            : ((program as any)?.status ??
                                (program as any)?.active) === "completed"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
                  ...
                  {mode === "view" ? (
                    <div className="h-12 flex items-center px-4 bg-gray-800 rounded-lg text-white">
                      {reschedules[0]?.end_date
                        ? new Date(reschedules[0].end_date).toLocaleDateString()
                        : "N/A"}
                    </div>
                  ) : (
                    <input
                ...
                <div className="mt-8 pt-8 border-t border-gray-700">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-400 text-sm">
                      Track and manage session reschedules across all mentors
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
                      ...
                      <div className="text-2xl font-bold text-white">
                        {mentorReschedules?.data?.total_reschedules ?? 0}
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
                      ...
                      <div className="text-sm text-red-400">
                        {mentorReschedules?.data?.top_mentor?.reschedules
                          ? `${mentorReschedules.data.top_mentor.reschedules} reschedules`
                          : ""}
                      </div>
                    </div>
                      ...
                      <div className="text-2xl font-bold text-white">
                        {
                          mockReschedules.filter((r) => {
                            const rescheduleDate = new Date(
                              r.rescheduledDateTime
                            );
                            const now = new Date();
                            const daysAgo =
                              (now.getTime() - rescheduleDate.getTime()) /
                              (1000 * 60 * 60 * 24);
                            return daysAgo <= 7;
                          }).length
                        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="reschedules-desc">
                          Most Reschedules
                        </option>
                        <option value="reschedules-asc">
                          Least Reschedules
                        </option>
                      </select>
                    </div>
